...
/**
 * This class stores and provides thread safe access to the odometer data. Modified by Rintaro to
 * also store the sensor reading (distance) from the US sensor. Also to support the negative value
 * in theta.
 * 
...
public class OdometerData {
  ...
  // US Sensor value parameter
  private volatile int distance;

  // Class control variables
  private volatile static int numberOfIntances = 0; // Number of OdometerData objects instantiated
  // so far
  ...
  /**
   * Returns the current reading stored in the distance.
   * 
   * @return stored sensor reading (distance in cm)
   */
  ...
  /**
   * Overrides distance. Use for obstacle avoider.
   * 
   * @param distance the value of US sensor value
   */
